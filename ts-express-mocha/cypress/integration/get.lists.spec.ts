...
describe('Get Lists from User Spec', () => {
    ...
    const userOwner: any = users[1]
    const userNotOwner: any = users[2]
    const basePath: string = `/users/${userOwner['id']}/lists`

    const addSongsPayload: any = {
        list: {
            songs: [
                {
    ...
    it('should return 401 if the user id does not exist', () => {
        const basePath: string = '/users/fake/lists'
